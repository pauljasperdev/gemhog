...
**Critical:**
- tRPC 11.8.1 - End-to-end type-safe API (`packages/api/`, `apps/web/`, `apps/server/`)
...
- @polar-sh/better-auth 1.6.4 - Better Auth Polar plugin (`packages/auth/`, `apps/web/`)
...
- Server: `DATABASE_URL`, `BETTER_AUTH_SECRET`, `POLAR_ACCESS_TOKEN`, `CORS_ORIGIN`
- Client: `NEXT_PUBLIC_SERVER_URL`

**Build:**
- `tsconfig.json` - TypeScript config (extends `@gemhog/config/tsconfig.base.json`)
