...
7. Update `packages/env/src/web.ts`:
   - Add NEXT_PUBLIC_SENTRY_DSN to client env schema (optional string, since we may not want Sentry in local dev)
  </action>
  <verify>
...
- next.config.ts exports withSentryConfig wrapped config
  ...
  <done>
Sentry SDK installed and configured. Client, server, and edge configs ready. Next.js build will upload source maps when SENTRY_AUTH_TOKEN is set.
  </done>
</task>
