...
files_modified:
  ...
  - apps/web/src/lib/sentry/instrumentation.ts
  - apps/web/src/lib/sentry/instrumentation.client.ts
  - apps/web/src/lib/sentry/server.config.ts
  - apps/web/src/lib/sentry/edge.config.ts
  ...
  - service: sentry
    ...
    env_vars:
      - name: SENTRY_AUTH_TOKEN
        source:
          "Sentry Dashboard -> Settings -> Auth Tokens -> Create New Token (with
...
must_haves:
  truths:
    ...
    - "Error boundaries display user-friendly fallback UI (production) or stack
      trace (dev)"
  artifacts:
    - path: "apps/web/src/instrumentation.ts"
      provides: "Server/edge entry for SDK registration"
      contains: "register"
    - path: "apps/web/src/instrumentation-client.ts"
      provides: "Client entry for SDK initialization"
      contains: "onRouterTransitionStart"
    - path: "apps/web/src/lib/sentry/instrumentation.ts"
    ...
    - path: "apps/web/src/lib/sentry/server.config.ts"
      provides: "Server-side Sentry config"
      contains: "Sentry.init"
    - path: "apps/web/src/lib/sentry/edge.config.ts"
    ...
    - path: "apps/server/src/index.ts"
      ...
      contains: "initSentry"
  key_links:
    - from: "apps/web/src/lib/sentry/instrumentation.ts"
      to: "apps/web/src/lib/sentry/server.config.ts"
    ...
    - from: "apps/web/src/lib/sentry/server.config.ts"
...
Purpose: Establish error visibility before building more features. Errors from
frontend, backend, and edge runtime will be captured and appear in the Sentry
dashboard with readable stack traces.
...
- Error boundaries showing user-friendly UI in production, stack traces in
  development </objective>

<execution_context> @/home/lima/.claude/get-shit-done/workflows/execute-plan.md
@/home/lima/.claude/get-shit-done/templates/summary.md </execution_context>
...
@/home/lima/.claude/skills/vercel-react-best-practices/SKILL.md </context>
...
2. Create `apps/web/src/lib/sentry/instrumentation.ts`:
   - Export async `register()` function
   - Conditionally import `server.config.ts` for nodejs runtime
   - Conditionally import `edge.config.ts` for edge runtime
   - Export `onRequestError = Sentry.captureRequestError`

3. Create `apps/web/src/lib/sentry/instrumentation.client.ts`:
   - Import env from `@gemhog/env/web`
   - **CRITICAL: Add DSN guard to skip initialization when DSN is not
     configured:**
     ...
     if (!env.NEXT_PUBLIC_SENTRY_DSN) {
       ...
       if (process.env.NODE_ENV === "development") {
         console.info(
           "Sentry DSN not configured, skipping client initialization",
         );
       }
     } else {
       Sentry.init({
   ...
   - Configure session ID tagging (generate UUID via crypto.randomUUID(), store
     in sessionStorage)
   ...
   - Configure replaysSessionSampleRate: 0, replaysOnErrorSampleRate: 0 (free
     tier)
   - Add ignoreErrors patterns from RESEARCH.md (ResizeObserver, ChunkLoadError,
     etc.)
   - Add denyUrls for browser extensions (chrome-extension://, moz-extension://,
     etc.)
   - Enable breadcrumbs integration (console, dom, fetch, history)

4. Create `apps/web/src/instrumentation.ts`:
   - Re-export `register` and `onRequestError` from
     `./lib/sentry/instrumentation`

5. Create `apps/web/src/instrumentation-client.ts`:
   - Re-export `onRouterTransitionStart` from
     `./lib/sentry/instrumentation.client`

6. Create `apps/web/src/lib/sentry/server.config.ts`:
   ...
   - **CRITICAL: Add DSN guard to skip initialization when DSN is not
     ...
     if (!env.SENTRY_DSN) {
       if (process.env.NODE_ENV === "development") {
         console.info(
           "Sentry DSN not configured, skipping server initialization",
         );
       }
     } else {
       Sentry.init({
...
7. Create `apps/web/src/lib/sentry/edge.config.ts`:
   ...
   - **CRITICAL: Add DSN guard to skip initialization when DSN is not
     ...
     if (!dsn) {
       if (process.env.NODE_ENV === "development") {
         console.info(
           "Sentry DSN not configured, skipping edge initialization",
         );
       }
     } else {
       Sentry.init({
...
8. Update `apps/web/next.config.ts`:
...
9. Update `packages/env/src/web.ts`:
   - Add NEXT_PUBLIC_SENTRY_DSN to client env schema as **optional string**:
     ...
     ```
   - This ensures builds succeed without Sentry configured (for tests and local
     dev)
...
10. Update `packages/env/src/server.ts`:
    - Add server-side Sentry env vars as **optional strings**:
      ...
      ```
    - All Sentry vars are optional because Sentry only runs in production (not
      local dev)
...
12. Summary of DSN guards (already covered in steps 3-5):

- `instrumentation-client.ts`: Uses `env.NEXT_PUBLIC_SENTRY_DSN` from
  `@gemhog/env/web`
- `src/lib/sentry/server.config.ts`: Uses `env.SENTRY_DSN` from
  `@gemhog/env/server`
- `src/lib/sentry/edge.config.ts`: Uses `process.env.SENTRY_DSN` directly (edge
  runtime limitation)
- All vars are optional in env schemas, so validation passes when not set
- This ensures Next.js build succeeds without SENTRY_DSN set (critical for
  startup.int.test.ts and local dev) </action> <verify>
- `pnpm check` passes (no TypeScript errors)
- Files exist: instrumentation.ts, instrumentation-client.ts,
  src/lib/sentry/server.config.ts, src/lib/sentry/edge.config.ts
- next.config.ts exports withSentryConfig wrapped config
- Build succeeds without SENTRY_DSN set: `pnpm test` passes (includes
  startup.int.test.ts) </verify> <done> Sentry SDK installed and configured.
  Client, server, and edge configs ready with DSN guards to skip initialization
  when not configured. Next.js build will upload source maps when
  SENTRY_AUTH_TOKEN is set. Build succeeds without Sentry env vars for tests and
  local dev. </done> </task>
...
3. Create `apps/web/src/components/section-error-boundary.tsx`:
   - "use client" directive
   - Import Sentry from @sentry/nextjs
   - Create SectionErrorBoundary component with props: children, section
     (optional string), fallback (optional ReactNode)
...
- Keep error boundary components minimal and focused </action> <verify>
- `pnpm check` passes
- All three files exist with "use client" directive
- Each file imports @sentry/nextjs and calls captureException </verify> <done>
  Error boundaries created: global-error.tsx for root errors, error.tsx for
  app-level, section-error-boundary.tsx for reusable section-level boundaries.
  All boundaries report to Sentry and show appropriate UI based on environment.
  </done> </task>
...
Note: The user will need to set these secrets via `sst secret set` after
deployment config is updated. Source map upload requires SENTRY_AUTH_TOKEN
during build, which SST handles by injecting env vars at build time. </action>
<verify>

- `pnpm check` passes (infra files may not be type-checked in main config,
  verify no syntax errors)
- infra/secrets.ts contains SentryDsn, SentryAuthToken, SentryOrg, SentryProject
  exports
- infra/web.ts references all Sentry secrets in environment block
- infra/api.ts references SentryDsn in environment block </verify> <done> SST
  infrastructure updated with Sentry secrets. Both Next.js (web) and Hono (api)
  will have access to Sentry DSN when deployed. </done> </task>
...
4. Verify Hono server still starts locally without SENTRY_DSN:
   ...
   - Console should show "Sentry DSN not configured" in dev mode only </action>
     <verify>
...
- Error handler registered after routes </verify> <done> Hono server now
  captures errors to Sentry via dedicated sentry.ts module. Index.ts stays clean
  with just imports and function calls. </done> </task>
...
Note: Full production verification requires deployment. This checkpoint
validates the local integration works. </how-to-verify> <resume-signal>Type
"approved" if error boundaries work locally, or describe any
issues.</resume-signal> </task>

</tasks>

<verification>
**After each task:**
- Static analysis + types: `pnpm check`

**Before marking plan complete:**

1. Full test pipeline: `pnpm test`
2. Dev servers start without errors:
...
- Frontend: Verify console shows "Sentry DSN not configured, skipping
  initialization"
...
- Trigger backend error → appears in Sentry with request context </verification>

<success_criteria> **Next.js (apps/web):**

1. @sentry/nextjs installed in apps/web
2. Client SDK initializes with session ID tagging and filtering (when DSN
   configured)
...
**Hono Server (apps/server):** 8. @sentry/node installed in apps/server 9.
Sentry middleware captures errors with request context 10. Server starts without
SENTRY_DSN (graceful skip)

**Infrastructure:** 11. SST secrets configured for Sentry credentials (web +
api) 12. Env package updated: SENTRY vars optional in both web.ts and
server.ts 13. .env.example documents all Sentry vars

**Verification:** 14. Both apps run locally without Sentry (`pnpm dev` works
without SENTRY_DSN) 15. `pnpm test` passes (full pipeline including
integration + E2E) </success_criteria>
