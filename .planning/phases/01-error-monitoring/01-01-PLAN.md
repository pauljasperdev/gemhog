...
3. Create `apps/web/instrumentation-client.ts`:
   - **CRITICAL: Add DSN guard at the top of the file to skip initialization when DSN is not configured:**
     ```typescript
   ...
   - Initialize Sentry with DSN from `process.env.NEXT_PUBLIC_SENTRY_DSN`
...
4. Create `apps/web/sentry.server.config.ts`:
   - **CRITICAL: Add DSN guard at the top to skip initialization when DSN is not configured:**
     ```typescript
     ...
     ```
   - Only proceed with Sentry.init if DSN exists
   - Initialize Sentry with DSN from `process.env.SENTRY_DSN`
   - Set environment from `process.env.NODE_ENV`
   - Configure tracesSampleRate: 1.0 in dev, 0.1 in production
...
5. Create `apps/web/sentry.edge.config.ts`:
   - **CRITICAL: Add DSN guard at the top to skip initialization when DSN is not configured:**
     ...
     const dsn = process.env.SENTRY_DSN;
     if (!dsn) {
       console.warn("Sentry DSN not configured, skipping edge initialization");
       export {};
       return;
     }
     ```
...
8. Ensure all Sentry initialization files have DSN guards:
   - `instrumentation-client.ts`: Check `process.env.NEXT_PUBLIC_SENTRY_DSN` before `Sentry.init()`
   - `sentry.server.config.ts`: Check `process.env.SENTRY_DSN` before `Sentry.init()`
   - `sentry.edge.config.ts`: Check `process.env.SENTRY_DSN` before `Sentry.init()`
   - Pattern for each file:
     ```typescript
     const dsn = process.env.NEXT_PUBLIC_SENTRY_DSN; // or SENTRY_DSN for server/edge
     if (!dsn) {
       // Log warning in development only
       if (process.env.NODE_ENV === "development") {
         console.warn("Sentry DSN not configured, skipping initialization");
       }
       // Don't call Sentry.init() - early return or skip the init block
     }
     ```
   - This ensures Next.js build succeeds without SENTRY_DSN set (critical for startup.int.test.ts)
  </action>
  <verify>
...
4. Dev server starts without errors: `pnpm dev:web`

Manual verification:
- Trigger a test error in the browser
- Error boundary displays fallback UI
- (With DSN configured) Error appears in Sentry dashboard
</verification>

<success_criteria>
1. @sentry/nextjs installed in apps/web
2. Client SDK initializes with session ID tagging and filtering
3. Server SDK initializes for nodejs and edge runtimes
...
9. `pnpm verify:commit` passes (check + types + unit tests)
</success_criteria>
