...
must_haves:
  ...
  artifacts:
    ...
    - path: "apps/server/src/index.ts"
      provides: "Hono server with Sentry error capture"
...
2. Update `apps/server/src/index.ts`:
   - Import Sentry at the top: `import * as Sentry from "@sentry/node";`
   - Import env: `import { env } from "@gemhog/env/server";`
   - **Add DSN guard and initialize Sentry BEFORE app creation:**
     ```typescript
     // Initialize Sentry (skip if DSN not configured)
     if (env.SENTRY_DSN) {
       Sentry.init({
         dsn: env.SENTRY_DSN,
     ...
     ```
   - Add Sentry error handler middleware to Hono app:
     ```typescript
...
3. Verify Hono server still starts locally without SENTRY_DSN:
   - `pnpm dev:server` should work without errors
   - Console should NOT show Sentry warnings (silent when not configured)
  </action>
  <verify>
- `pnpm check-types` passes
- `pnpm dev:server` starts without SENTRY_DSN set
- apps/server/src/index.ts imports @sentry/node
- Error handler middleware is registered after routes
  </verify>
  <done>
Hono server now captures errors to Sentry. Errors in API routes (tRPC, AI endpoint, auth) will appear in Sentry dashboard with request context.
  </done>
</task>
