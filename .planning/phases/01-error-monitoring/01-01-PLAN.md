...
- Build succeeds without SENTRY_DSN set: `pnpm test:unit` passes (includes
...
- `pnpm check-types` passes (infra files may not be type-checked in main config,
...
without SENTRY_DSN) 15. `pnpm verify` passes (full pipeline including
