...
**Deleted unused PaymentService, PaymentLive, polarClient exports - Polar
integration is handled inline in auth.service.ts via better-auth plugin**
...
- Removed dead payment service code (PaymentService, PaymentLive, polarClient,
  getPolarClient)
...
- `packages/core/src/payment/payment.service.ts` - Dead
  PaymentService/PaymentLive code
...
1. **Delete entire payment/ directory** - All exports (PaymentService,
   PaymentLive, polarClient, getPolarClient, PaymentError) were unused. No
   reason to keep an empty module.

2. **Keep @polar-sh/sdk dependency** - Although the payment module was deleted,
   auth.service.ts still uses `@polar-sh/sdk` to create a Polar client inline
   for the better-auth plugin.
...
1. **Git staging issue** - Initial commit only included package.json change, not
   the file deletions. The pre-commit hook restored the working directory state.
   Fixed by amending the commit to include all deleted files.
