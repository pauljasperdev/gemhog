...
key-decisions:
  - "Use inline rate limiting instead of external package (simpler for
    single-server)"
...
**Zod validation for AI messages with rate limiting (10 req/min) - fixes SEC-001
and SEC-002**
...
- `apps/server/src/index.ts` - Added Zod schemas, rate limiter, and validation
  in /ai endpoint
- `packages/core/src/auth/auth.service.ts` - Deferred env import for testability

## Decisions Made

1. **Inline rate limiting instead of package** - Simpler for single-server
   deployment, no external dependencies needed
2. **UIMessage parts validation** - AI SDK v6 uses `parts` array with
   `{type, text}` objects, not simple `{role, content}`
3. **Discriminated union for parts** - TextPartSchema, FilePartSchema,
   OtherPartSchema with passthrough for unknown types
4. **Fallback to "anonymous" for rate limiting** - If no IP headers present, use
   "anonymous" as identifier
...
- **Issue:** Plan assumed simple `{role, content}` message format but AI SDK v6
  uses UIMessage with `parts` array
- **Fix:** Created schema matching actual UIMessage format:
  `{id?, role, parts: [{type, text}], metadata?}`
- **Files modified:** apps/server/src/index.ts
- **Verification:** TypeScript compilation passes, validation works with
  frontend
...
- **Issue:** auth.service.ts had static import of env which triggered Effect
  Config validation at module load time, causing unit tests to fail
- **Fix:** Applied deferred require() pattern (established in 03.1-03) - env
  only loaded when getAuth() is called
...
fixes necessary for correctness - schema adjustment for proper validation, env
deferral for test isolation.

## Issues Encountered

- Pre-commit hook requires Biome import organization - applied auto-fix before
  commit
