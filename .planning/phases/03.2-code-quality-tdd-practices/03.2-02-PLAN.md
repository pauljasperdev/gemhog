...
must_haves:
  truths:
    ...
    - "Unit tests verify web env schema fails when NEXT_PUBLIC_SERVER_URL missing"
...
Purpose: Verify that the Effect Config schemas correctly fail when required environment variables are missing, and succeed when all required vars are provided. This catches env configuration errors at test time rather than runtime.

Output: `packages/env/src/server.test.ts` and `packages/env/src/web.test.ts` with comprehensive schema validation tests.
</objective>
...
Use Effect ConfigProvider.fromMap to provide isolated env vars without touching process.env.
...
Important: Do NOT import from `./server.ts` directly - that would trigger validation with real env vars. Instead, recreate the Config definition in the test file for isolated testing.
