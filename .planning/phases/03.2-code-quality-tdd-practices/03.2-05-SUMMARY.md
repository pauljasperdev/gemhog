...
key-decisions:
  ...
  - "Use DOTENV_CONFIG_PATH=/nonexistent to bypass server .env loading"
...
patterns-established:
  ...
  - "Server env testing: set DOTENV_CONFIG_PATH to nonexistent file"
...
3. **Fix: Prevent dotenv loading in server tests** - `afc8dd4` (fix)
...
3. **Use DOTENV_CONFIG_PATH for server test** - Server uses `dotenv/config` which auto-loads .env; setting DOTENV_CONFIG_PATH to nonexistent file prevents this
...
3. **Server reads .env via dotenv** - Server tests initially passed because `dotenv/config` imported in packages/env loaded apps/server/.env automatically. Resolved by setting DOTENV_CONFIG_PATH to a nonexistent file path.
