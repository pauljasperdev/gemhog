...
**Code type:** CLI tooling (drizzle-kit) — NOT Effect runtime code.
drizzle-kit is development/deployment tooling. @effect/sql-drizzle is for runtime queries.
Testing migrations uses direct drizzle-orm `migrate()` function, which is appropriate for CLI tooling tests.
</objective>
