...
**Code type:** Edge code (better-auth) — no Effect wrapper, direct testing is appropriate.
better-auth has no Effect wrapper and lives at the HTTP boundary. Testing directly via `getAuth()` API is the correct approach. The auth layer will remain non-Effect as it's intentionally at the edge of the application.
</objective>
