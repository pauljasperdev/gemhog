...
**Code type:** Edge code (tRPC router) — thin API wrapper, direct testing is appropriate.
tRPC is a thin wrapper layer for the API. It's not Effect code internally (though it may call Effect code in the future). Testing via `createCaller()` is the standard tRPC testing pattern and is correct for edge/boundary code.
</objective>
...
<task type="auto">
  ...
  <action>
Create integration tests for all tRPC procedures using the modern createCallerFactory pattern.
...
3. Import t from "../index" (the tRPC instance)
4. Import TRPCError from "@trpc/server"

**Use modern createCallerFactory pattern** (from RESEARCH.md):
```typescript
import { t } from "../index";
...
**Note:** Use `t.createCallerFactory(appRouter)` instead of the deprecated `appRouter.createCaller()`. The deprecated API still works but createCallerFactory is the recommended tRPC v11 pattern.
  </action>
