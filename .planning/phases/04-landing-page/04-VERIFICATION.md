...
**Phase Goal:** Visitors see a compelling marketing page and can subscribe to
the newsletter
...
- `pnpm vitest run signup-form.test.tsx landing-footer.test.tsx` — **8/8 tests
  PASSED**
  - SignupForm: 5 tests (render, privacy link, success state, loading state,
    error state)
...
- **Issue:** Commit 9ce32db hoisted `EmailLayers` construction to module scope
  in `subscriber.ts`, causing `env.RESEND_API_KEY` to be accessed at module
  import time
- **Impact:** Integration tests timing out during `pnpm build` because env vars
  accessed before validation
